...
def foto_upload_section(key_prefix, max_foto=3):
    ...
    st.markdown(f"**📷 Foto ({len(temp_list)}/{max_foto})**")
    ...
    if len(temp_list) < max_foto:
        cam_shot = st.camera_input(
            ...
            help="Scatta una foto con la fotocamera del dispositivo"
        )
...
if page == "Anagrafica Bombole":
    ...
    # Sezione foto bombola
    foto_list_bom = foto_upload_section("bombola_new", max_foto=3)
    ...
    if movs:
        ...
        for idx, row in df_mov.iterrows():
            with st.container(border=True):
                ...
                if foto_mov:
                    with st.expander(f"📷 Foto ({len(foto_mov)})", expanded=False):
                        render_foto_gallery(foto_mov, caption_prefix="Foto")
        st.divider()
