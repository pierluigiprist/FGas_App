...
st.markdown("""
<script>
    // Meta tags per PWA (Aggiungi alla schermata home)
    ...
    if (!document.querySelector('meta[name="theme-color"]')) {
        ...
        m3.name = 'theme-color'; m3.content = '#0D47A1';
        document.head.appendChild(m3);
    }
    if (!document.querySelector('meta[name="mobile-web-app-capable"]')) {
        const m4 = document.createElement('meta');
        m4.name = 'mobile-web-app-capable'; m4.content = 'yes';
    ...
    const manifest = {
        ...
        icons: [
            {src: "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 192 192'%3E%3Crect fill='%230D47A1' width='192' height='192' rx='24'/%3E%3Ctext x='96' y='120' font-size='100' text-anchor='middle' fill='white'%3E🔧%3C/text%3E%3C/svg%3E", sizes: "192x192", type: "image/svg+xml"}
        ]
    };
...
def render_form_da_qr(payload):
    ...
    with st.form("form_mov_qr", clear_on_submit=True):
        ...
        if submitted:
            # Controllo quantità disponibile per CARICA
            bombola_qr = None
            for b in st.session_state.data["bombole"]:
