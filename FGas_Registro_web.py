...
GAS_COLORS = {"R32": "#1565C0", "R410": "#7B1FA2", "R407": "#F57C00", "R424": "#2E7D32", "Misto": "#C62828"}
...
if page == "Anagrafica Bombole":
    ...
    with st.form("form_bombola", clear_on_submit=True):
        ...
        with c1:
            ...
            id_bom = st.text_input("ID Interno")
        ...
        with c2:
            ...
            cap_lt = st.number_input("Capacita Max (Lt)", min_value=0.0, step=0.1, format="%.1f")
        with c3:
            cap_kg = st.number_input("Capacita Max (Kg)", min_value=0.0, step=0.1, format="%.1f")
            carico = st.text_input("In Carico a")
        ...
        with c5:
            ...
            stato_bom = st.selectbox("Stato", ["", "NOLEGGIO", "PROPRIA", "RESTITUITA", "DISMESSA", "In Uso", "In Attesa Ritiro"])
        with c6:
        ...
        if submitted and id_bom.strip():
            st.session_state.data["bombole"].append({
                ...
                "tara": tara, "cap_lt": cap_lt, "cap_kg": cap_kg,
                "in_carico_a": carico.strip(), "data_assegnazione": data_ass.strip(),
    ...
    if not df_bom.empty:
        edited_bom = st.data_editor(
            ...
            column_config={
                ...
                "stato": st.column_config.SelectboxColumn("Stato", options=["", "NOLEGGIO", "PROPRIA", "RESTITUITA", "DISMESSA", "In Uso", "In Attesa Ritiro"])
            }
        )
...
def render_movimentazione_page(tipo_gas):
    ...
    with st.form(f"form_mov_{tipo_gas}", clear_on_submit=True):
        ...
        st.markdown("**Foto della pesatura (scatta in tempo reale)**")
        foto_camera = st.camera_input("Scatta foto", key=f"cam_{tipo_gas}")
        ...
        if submitted:
            if quantita_mov <= 0:
                ...
                foto_b64 = img_to_b64(foto_camera)
                mov = {
                    "tipo_gas": tipo_gas,
