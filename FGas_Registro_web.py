...
if page == "Anagrafica Bombole":
    ...
    if not df_bom.empty:
        ...
        if not df_filt_bom.empty:
            ids_visibili = df_filt_bom["id_interno"].astype(str).tolist()
        ...
        tec_list = get_nome_tecnici()
        # Reset index per evitare problemi con st.data_editor
        df_editor = df_filt_bom.reset_index(drop=True)
