...
with st.sidebar:
    ...
    with st.expander("🗑️ Reset Matrice Bombole"):
    ...
    with st.expander("🗑️ Reset Movimentazioni"):
                ...
                for j, b in enumerate(gas_bombole[i:i+n_cols]):
                    with cols[j]:
                        ...
                        st.markdown(
                            ...
                            f'<div style="margin-left:auto;">{tipo_icon} <span style="{tipo_style}">{tipo.upper()}</span></div>'
                            f'</div>'
...
if page == "Scanner QR":
    ...
    st.markdown('<div class="sub-header">Registra una movimentazione scansionando il QR code direttamente dalla fotocamera</div>', unsafe_allow_html=True)
    st.divider()
    ...
    if payload:
        ...
        # ========== SCANNER FOTOCAMERA IN TEMPO REALE ==========
        st.subheader("📷 Scansiona con la fotocamera")
        st.caption("Inquadra il QR code della bombola con la fotocamera del dispositivo. I dati verranno letti automaticamente.")

        scanner_html = """
        <div style="text-align:center; font-family: Arial, sans-serif;">
          <div id="qr-reader" style="width:100%; max-width:520px; margin:0 auto; border-radius:16px; overflow:hidden; border:2px solid #e0e0e0;"></div>
          <div id="qr-status" style="margin-top:14px; font-weight:600; min-height:28px; font-size:1.05rem;"></div>
          <button id="btn-restart" style="display:none; margin-top:10px; padding:10px 24px; border-radius:10px; background:#0D47A1; color:white; border:none; cursor:pointer; font-size:1rem; font-weight:600;" onclick="restartScanner()">🔄 Riavvia Scanner</button>
        </div>
        ...
        function startScanner() {
            showStatus("⏳ Avvio fotocamera...", "#0D47A1");
            ...
            html5QrCode.start(
                ...
                isScanning = true;
                showStatus("📷 Inquadra il QR code della bombola...", "#2E7D32");
            }).catch(err => {
                showStatus("❌ Errore fotocamera: " + err, "#C62828");
        ...
        function onScanSuccess(decodedText, decodedResult) {
            ...
            isScanning = false;
            showStatus("✅ QR rilevato! Caricamento dati...", "#2E7D32");

            html5QrCode.stop().then(() => {
                var params = {};
                // 1) Prova a parsare come JSON
                try {
                    var obj = JSON.parse(decodedText);
                    if (obj.v && obj.v.startsWith("FGas")) {
                        params = {
                            qr_gas: obj.gas || "",
                            qr_id: obj.id || "",
                            qr_tipo: obj.tipo || "Cariche",
                            qr_qta: String(obj.qta || 0),
                            qr_seriale: obj.seriale || ""
                        };
                    }
                } catch(e) {
                    // 2) Prova a parsare come URL completo
                    try {
                        var url = new URL(decodedText);
                        var sp = url.searchParams;
                        if (sp.get("qr_gas") && sp.get("qr_id")) {
                            params.qr_gas = sp.get("qr_gas");
                            params.qr_id = sp.get("qr_id");
                            params.qr_tipo = sp.get("qr_tipo") || "Cariche";
                            params.qr_qta = sp.get("qr_qta") || "0";
                            params.qr_seriale = sp.get("qr_seriale") || "";
                        }
                    } catch(e2) {
                        // 3) Prova a cercare parametri in una stringa qualsiasi
                        var idx = decodedText.indexOf("?");
                        if (idx !== -1) {
                            var qs = new URLSearchParams(decodedText.substring(idx));
                            if (qs.get("qr_gas") && qs.get("qr_id")) {
                                params.qr_gas = qs.get("qr_gas") || "";
                                params.qr_id = qs.get("qr_id") || "";
                                params.qr_tipo = qs.get("qr_tipo") || "Cariche";
                                params.qr_qta = qs.get("qr_qta") || "0";
                                params.qr_seriale = qs.get("qr_seriale") || "";
                            }
                        }
                    }
                }

                if (params.qr_gas && params.qr_id) {
                    var url = new URL(window.top.location.href);
                    url.searchParams.set("qr_gas", params.qr_gas);
                    url.searchParams.set("qr_id", params.qr_id);
                    url.searchParams.set("qr_tipo", params.qr_tipo);
                    url.searchParams.set("qr_qta", params.qr_qta);
                    url.searchParams.set("qr_seriale", params.qr_seriale);
                    window.top.location.href = url.toString();
                } else {
                    showStatus("⚠️ QR non riconosciuto come FGas. Riprova.", "#E65100");
                    document.getElementById('btn-restart').style.display = 'inline-block';
                }
            });
        }
        ...
        function onScanFailure(error) {
            // Nessun QR nel frame, ignorato silenziosamente
        }

        startScanner();
        </script>
        """
        components.html(scanner_html, height=540)
